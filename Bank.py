...
def finish_reg():
    ...
    for name_check in all_accounts:
        if name ==name_check:
            ...
            new_file.write("0"+"\n")
            new_file.close()
            notif.config(fg="#35dd02", text="Account has been created")
...
def beneficiary():
     ...
     global beniciary_account
     beneficiary_amount= StringVar()
     beniciary_account=StringVar()
     file=open('Login Database.txt', "r")
     file_data = file.read()
     user_details=file_data.split('\n')
     details_balance=float(user_details[4])
     ...
     Label(beneficiary_screen,text="\nAccount No.\n",bg='black',fg='white', font=('Baskervill Old Face',12)).grid(row=4,sticky=W)
     Entry(beneficiary_screen,textvariable=beniciary_account).grid(row=4,column=0,sticky=E)
...
def login_session():
    ...
    while 'Login Database.txt' in all_accounts:
            ...
            name=file_data[0]
            if temp_login_name=="" and temp_password=="":
...
#for login screen
    ...
    Label(login_screen, text="Username:",bg='black',fg='white', font=('Baskervill Old Face', 12)).grid(row=2,sticky=W)
...
Label (bank_Window, image=img, borderwidth=0,bg='black').grid(row=1,sticky=N,pady=10, padx=50)
Label(text="", bg='black').grid(row=3)
Label(text="", bg='black').grid(row=4,pady=10)

Button(bank_Window, text="Sign up",cursor="hand2",bg='black',fg='white', font=('Baskervill Old Face', 12),width=10,command=register).grid(row=5,sticky=N,pady=10)
Button(bank_Window, text="Login", bg='black',fg='white',font=('Baskervill Old Face', 12),cursor="hand2" ,width=10,command=login).grid(row=6,sticky=N, pady=10)
